...
/** 성향 검사를 아직 하지 않은 사용자가 CTA로 가는 곳 */
const ONBOARDING_HREF = "/onboarding";
/** 이미 검사·로그인한 사용자는 홈으로 간다. 홈이 진행 중 여행·일정 정하기로 이어 준다. */
const HOME_HREF = "/";
 ...
 * CTA는 성향 검사를 아직 안 한 사용자를 검사(/onboarding)로, 마친 사용자를 홈으로 보낸다.
 */
const AboutFinal = () => {
  // 로그인했거나 검사 결과가 있으면 검사를 마친 사용자다.
  const isLoggedIn = useSessionStore((state) => state.isLoggedIn);
  const preferenceType = useSessionStore((state) => state.preferenceType);
  const ctaHref = isLoggedIn || preferenceType ? HOME_HREF : ONBOARDING_HREF;
